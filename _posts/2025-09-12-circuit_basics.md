...
``` cmd
    ...
    OUT ---------| N-MOSFET
                ...
                GND
```
开漏输出主要特征：内部无上拉，输出只有“低电平”和“高阻”两种状态，高电平必须靠外部上拉。

开漏输出仅使用了一个N-MOSFET实现。当N-MOSFET导通时，OUT直接接地；当N-MOSFET断开时，输出引脚悬空，处于高阻态。此时外接的上拉电阻将OUT拉高。
...
推完输出主要特征：主动推高、主动拉低，不用外接电阻。

推挽输出由一个P-MOSFET和一个N-MOSFET组成，这种互补对称结构的MOSFET构成的集成电路技术成为CMOS。它结合了N型和P型MOSFET，利用它们的互补特性来实现逻辑功能。P-MOSFET连接到VCC，N-MOSFET连接到GND，OUT是它们的中点。当P-MOSFET导通时，OUT被拉高；当N-MOSFET导通时，OUT被拉低。

两个管子互补工作，不会同时导通，因此输出可以有较强的驱动能力。这里MOSFET可以替换成三极管同样实现推挽效果，且成本较低，但不适合高频电路。
