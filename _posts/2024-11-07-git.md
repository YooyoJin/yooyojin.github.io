...
### 4.1 使用Git push时报错 git.exe push --progress "origin" main:main Connection reset by 20.205.243.166 port 22.
...
出现这个报错我以为是因为我刚改了git仓库的用户名，我开始认为是我仓库位置变了，会不会是ssh密钥账户匹配出了问题，所以无法访问远程仓库？

于是我尝试重新拉一个仓库，用ssh，成功拉下来了。但是再push改动一样报错。

网上擦了下，发现大家遇到这个问题，也尝试很多办法。
- ~~重新生成ssh密钥~~
- ~~修改22端口为443~~
- ~~修改git用户邮箱设置~~
...
于是我也将目标重新锁定回网络，发现自己开了机场，于是把机场关掉，push成功
