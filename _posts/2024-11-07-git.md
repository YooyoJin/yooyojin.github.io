...
# 删除标签
# 删除远程标签第一步先删除本地标签，第二步使用git push命令的 --delete 选项推送删除操作到远程仓库
cmd: git tag -d 标签名
cmd: git push origin --delete 标签名
```
...
写完代码后，为了整理零散提交记录，于是对提交记录进行整合。图方便使用TortoiseGit处理，操作过过程中准备合并，本来是计划将两次提交合并成一个提交，但是不知道是出了bug还是怎样，跟平时弹出的处理窗口不一样。我也没注意直接跳过了，结果我的提交记录莫名其妙回到最初改动，看代码也已经回退了。瞬间汗流浃背了。。

这个场面还是第一次处理，而且没有手动备份，马上先备份了一下，但是小海龟上完全没有我的操作记录（其实就在show log下面，只是平时从来没用过），本来以为凶多吉少了，搜索后发现有类似问题，参考哗啦啦的yu的文章解决[^3]

本质上就是我的代码还在本地，虽然提交了commit但是没有提交到远程仓库。当执行了reset或切换分支后git log就找不到之前的commit记录了（git log只显示当前HEAD可达的提交，reset后 HEAD移动了，所以看不到）。

这里的关键，查看历史操作:

``` cmd
cmd：git reflog  # 这个记录了所有HEAD变更历史（每次commit、reset、checkout、merge、rebase都会被记录），这些记录会保留一段时间（默认90天）

Tortoise：右键 -> TortoiseGit -> Show Reflog

$ git reflog
df94c77 (HEAD -> master) HEAD@{0}: reset: moving to df94c7753803a58aedcc1d8037abad5b2edc433c
b3d9917 HEAD@{1}: commit: <feature> 某硬件小板适配，包括IO点位、功能、BSP修改（表驱动形式）
df94c77 (HEAD -> master) HEAD@{2}: commit: <feature> GBK改UTF8
3cc4ad6 HEAD@{3}: commit: <feature> 新增内容
cc628f1 HEAD@{4}: commit (amend): <feature> 针对硬件型号区分工程管理
747d27d HEAD@{5}: commit: <feature> 硬件管理
5098a68 (origin/master, origin/HEAD) HEAD@{6}: commit: <fix> 同步编译配置改动
622118e HEAD@{7}: commit: <feature> 增加自动编译脚本
9eba305 HEAD@{8}: commit: <feature> 实现cmake+gcc+jlink编译调试功能
4ef4879 HEAD@{9}: commit: <fix> 适配readme.md
4580ddc HEAD@{10}: commit: <feature> 初版
a77e532 HEAD@{11}: clone: from https://gitee.com/yourname/your-project.git
```

可以看到`HEAD@{0}`这一行——我刚执行了reset，现在HEAD指向了df94c77，而我要找的b3d9917已经不在当前分支的历史里了。

虽然这个提交不再当前分支历史里，但它仍存在与Git的数据库种，只要reflog里有记录，就还能恢复。

恢复方法：直接reset回去:

``` cmd
git reset --hard b3d9917
```

如果误操作了且没有发现，可能需要使用cherry-pick命令：

``` cmd
git cherry-pick b3d9917  #这会把b3d9917的改动作为一个新补丁应用到当前HEAD上。
```

总结：在不确定操作会造成什么后果时，记得先备份！
...
[^1]: Wrysmile0308. (2023). _使用Git时报错Connection reset by 20.205.243.166 port 22_. [https://blog.csdn.net/Wrysmile0308/article/details/128801870](https://blog.csdn.net/Wrysmile0308/article/details/128801870)

[^2]: fK0pS. (2019). _解决 Failed to connect to github.com port 443:connection timed out_. [https://blog.csdn.net/Hodors/article/details/103226958](https://blog.csdn.net/Hodors/article/details/103226958)

[^3]: 哗啦啦的yu. (2025). _Git提交丢失？别慌！手把手教你用git reflog找回“消失的代码”_. [https://juejin.cn/post/7478893378035433508](https://juejin.cn/post/7478893378035433508)