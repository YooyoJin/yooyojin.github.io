...
### 如何完成vscode，调试编译一体准备工作

如何自动生成 c_cpp_properties.json：
- `shift+ctrl+p --> C/C++:Edit Configurations(JSON)`

如何自动生成 launch.json：
- `运行 --> 添加配置 --> Cortex Debug`

如何配置Build，自动运行CMake：
- `右键CMakeList.txt --> 使用CMake配置所有项目 --> GCC arm-none-eabi --> 选择CMakeList.txt`

再根据项目修改相应配置，即可在vscode上面愉快的编译调试啦。
