...
- MSB（Most Significant Bit） = 最高有效位（数值最大的位，通常是bit 7或bit 15），MSB的传输数据顺序：11110001，先传高位；
- LSB（Least Significant Bit） = 最低有效位（数值最小的位，通常是bit 0），LSB的传输数据顺序：10001111，先传低位；
