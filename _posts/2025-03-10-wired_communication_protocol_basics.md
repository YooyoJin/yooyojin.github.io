...
- 采用TTL电平；
- 协议标准明确规定，每个字节从最高位开始传输，以最低位结束。是最典型的MSB First协议；

这里插一下位序概念（数据传输时的位顺序），分为了MSB和LSB两种。这里假设我们要传输一个8位数据：0b11110001（bit7 -> bit1，权重左边高，所以所以左边是高位）
- MSB（Most Significant Bit） = 最高有效位（数值最大的位，通常是bit 7或bit 15），MSB First的传输数据顺序：11110001，先传高位；
- LSB（Least Significant Bit） = 最低有效位（数值最小的位，通常是bit 0），LSB First的传输数据顺序：10001111，先传低位；
...
- 主机等待应答：主机释放数据线（开漏输出：主机输出高电平时，实际是断开下拉MOS管，由上拉电阻控制电平）。

**Q: 等待应答过程中，释放SDA原理，为什么这里SDA可以保持和拉低？<br>**
**A:** 首先我们知道了I2C是支持多主从的通信方式，他的物理连接如下：
...
从接线我们中，我们可以看到设备上所有的SDA引脚都在同一根线上，实现释放总线的的关键在于，每个设备都是开漏输出且总线上有上拉电阻。

开漏输出是由一个CMOS管来驱动，`我的代码 → GPIO输出寄存器 → MOS管 → 实际SDA引脚`，所以当我的设备配置GPIO为高电平时，MOSFET断开，意味着SDA引脚什么都不接，上拉电阻把SDA拉高，这时就是释放总线。

当有任意设备，输出低电平，MOSFET闭合， SDA总线直接接到GND，从而拉低总线，实现从机应答。
...
I2C的容错率比较高，协议允许在SCL低电平时，无限期暂停，时钟拉伸是正常行为。

只要SCL高时SDA稳定，数据就安全有效。例如：

``` cmd
传输字节 0xA5 (10100101b):
SCL: _/‾\_/‾\_/‾\________/‾\_/‾\_/‾\_/‾\_/‾\  // 第4位时钟被拉长
     1  0  1  0  暂停   0  1  0  1
     ↑  ↑  ↑  ↑         ↑  ↑  ↑  ↑
     MSB      第4位被拉长         LSB
```
...
详细I2C应用可以参考[EEPROM驱动开发笔记（M24C32）](./2025-06-19-eeprom_case.html)章节。
...
**A:** 在实际工程中，RS-485通常配合Modbus使用，表现为轮询机制；而CAN在硬件链路层就原生支持多主竞争机制，即使没有应用层协议，其通信行为也是基于仲裁的竞争机制。
<div style="overflow-x: auto;">
    <style>
    td {
        border: 1px solid #000;
...
CAN的OSI七层网络主要集中在物理层和数据链路层，数据链路层现行的标准协议：ISO 11898-1。

当然也有标准的应用层（CAN OPEN）、网络层（ISO 15765）等等，但是就目前的工作，没有应用的场景，要么只能自己创造条件，要么只能放弃这部分内容。
