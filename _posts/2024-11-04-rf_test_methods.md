...
## 4. 射频信号发生器（RF Signal Generators）
...
### 设备连接问题，IP配置错误导致设备找不到的

像我们常用的网分、频谱仪都自带操作系统，支持用PC控制，通常用LAN端口进行通信并控制。

步骤：
1. 使用标准网线连接仪器与电脑。观察网口指示灯是否亮起；
1. 改IP，<b>注意需要仪器和PC在同一个网段下，且末尾IP不同才可以通信！<b>
    - 查仪器IP，通常如果是windows系统的仪器，改IP的操作就跟windows一样。如果是其他系统，通常也也可以直接在仪器触摸屏面板上进行修改。
    - 改电脑IP
        1. 打开电脑 “网络设置” → “更改适配器选项”；
        1. 右键点击有线网卡 → 属性 → Internet协议版本4(TCP/IPv4)；
        1. 填入一个与仪器同网段（前三段相同）且末位不同的IP，通常就是IP地址：`192.168.1.1`,子网掩码：`255.255.255.0`；
        1. 点击“确定”保存。
1. 这时后通过打开NI MAX，右键“网络设备” → “查找网络NI-DAQmx设备”。通常可以识别到设备了。注意此处需要有NI环境；
1. Ping测试：在电脑命令提示符输入ping [仪器IP]，看到“来自...的回复”就表示ping通了；
1. 如果有VISA库，也可以直接用代码来控制仪器仪表了。VISA是NI的产品，基本上是市面上的仪器都支持VISA。

ps:IO Trace也非常好用，可以记录调用了哪些VISA指令，调试，排故必不可少。

## 参考资料

Teradyne. RF Fundamentals.