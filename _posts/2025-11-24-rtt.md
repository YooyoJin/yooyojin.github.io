...
### 在使用vsnprintf、snprintf标准库后，提示_write、_sbrk等系统函数未定义告警

vsnprintf、snprintf是C标准库函数，我这面直接用了。但是实际上他们底层依赖了一些操作系统的函数像_write、_sbrk等，但是我单片机又没有。一般这些是系统内核实现的。

PC(windos、Linux)与MCU的数据流向以及数据处理方式对比
```
    代码 (PC)                           代码 (MCU)
        ↓                                   ↓
    printf("Hello")                     C库 vsnprintf("Hello")
        ↓                                   ↓
    C库 vsnprintf("Hello")              _write() 没用，打桩实现
        ↓                                   ↓
    _write() → 操作系统系统调用          SEGGER_RTT_Write(buffer)
        ↓                                   ↓
    内核驱动程序                         RTT控制块(内存中)
        ↓                                   ↓   SWD
    直接写入显存/终端缓冲区               调试器J-Link
        ↓                                   ↓   USB
    你在PC屏幕上看到"Hello"              PC上的SEGGER RTT Viewer
                                            ↓
                                        你在PC屏幕上看到"Hello"
```

解决办法：提供系统装函数、或实现vsnprintf的可变参数处理功能。重复造轮子且这个轮子不好弄，我选择直接打桩，经过测试本质上是没有用系统函数的所以并没有影响。
