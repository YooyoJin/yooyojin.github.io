...
整个中断运行流程总结起来大致如下：
- 初始化：先配NVIC（中断向量控制器），再开外设中断
- 触发：外设发信号给NVIC
- 响应：CPU暂停手头活，自动保存现场
- 执行：进ISR（中断服务程序），先清标志，再处理中断服务函数（这里要快）
- 返回：自动恢复现场，接着干之前的活

中断概念相当于“一次电话通知”，你正在码代码，快递到了电话一响（中断触发），你放下手头工作（保存现场），去取快递（执行ISR），回来继续码代码（恢复现场）。

### 6.1. 简析中断系统

正常情况下，比较老的内核，需要用汇编手动处理压栈、出栈。但是Cortex-M4通过了硬件机制实现了压栈出栈动作，没有额外开销，并且可以用标准C语言实现`void XXX_Handler(void) { ... }`中断服务函数。意思就是触发中断时，硬件处理了现场保存，服务函数处理完，硬件再把指令地址、运行状态和关键计算数据等数据从栈里取出来，给到CPU寄存器（R0、R1、PC、xPSR 等），来恢复CPU状态。这里栈式后进先出的，在中断嵌套情况下也不受影响，恢复的都是上层被打断的状态。

关于中断的几个重要概念，中断源，中断向量表，中断优先级，中断的嵌套
- 中断源：仍用电话的例子来说，就是谁打来的电话？外设被设计具备打电话的能力。
    - 外部中断：按键、脉冲电平触发；
    - 内部中断：定时器中断、通信中断、DMA中断等。
- 中断向量表：就是电话簿。电话来了，看下是谁打的。如果快递打来的就去拿快递，如果是半夜老板打来的，就告诉他我%@#$。这张表里依次存放着所有中断服务函数的入口地址（函数指针）。当某个中断发生时，CPU根据中断号，直接在这个表里偏移找到对应的函数地址，然后跳转执行。
    - 中断被接受后，CPU 跳转到对应的中断服务函数执行；
    - 这个函数的名字必须是固定的！与中断向量表里面名称相同，芯片启动时，内存中有一张中断向量表，里面按顺序存着每个中断对应的函数入口地址；
    - 实际就是弱引用，启动文件预定义了一个空的ISR，你在写一个同名函数，就可以替换它。
- 中断优先级，就是谁有资格打断？比如你正在跟老板打电话，现在客户也打电话过来，那就让老板先滚一下，先处理客户的事情。
    - 抢占优先级决定是否可以打断；
    - 子优先级决定谁先响应；
    - 若都相同则比较中断编号，抢占优先级高的可以打断低的；只有抢占优先级相同时，才比较子优先级（且子优先级高者先响应，但不打断）。

> 除了中断请求，还有其他需要服务的事件，将其他称为“异常”。中断也是一种异常。

### 6.2. NVIC子系统常用API

- __disable_irq() / __enable_irq()：操作PRIMASK寄存器。拉总闸，屏蔽除HardFault和NMI外的所有中断。通常用于操作flash、涉及时序的关键代码。
- NVIC_DisableIRQ() / EnableIRQ()：操作NVIC的使能寄存器。关分闸，只屏蔽指定的某一个外设中断。
- NVIC_SetPriorityGrouping() 设置优先级分组（决定抢占优先级和子优先级各占几位）
- NVIC_SetPriority() 某个具体中断的优先级数值，包含抢占优先级和子优先级。
- NVIC_DisableIRQ(IRQn) 在ISR中调用NVIC_DisableIRQ(IRQn)关掉当前中断。执行完后退出ISR，程序恢复正常运行；不会再触发该中断，直到手动调用NVIC_EnableIRQ()重新开启。

### 6.3. 中断常用寄存器

SCB（系统控制模块）
- VTOR（向量表偏移寄存器），App的向量表需要偏移到Flash非起始位置用到了VTOR寄存器。通常要求 VTOR 的值必须是 0x200 的倍数（512 字节对齐）。
- AIRCR，设置优先级
