require(['gitbook', 'jQuery'], function (gitbook, $) {
    ...
    gitbook.events.bind('page.change', function () {
		...
		function updateDividerPosition() {
			...
			var windowHeight = $($summary).height();
			var dividerHeight = $divider.outerHeight();
			var topPosition = scrollTop + (windowHeight / 2) - (dividerHeight / 2);
			// console.log('scrollTop: ' + scrollTop + ', topPosition: ' + topPosition);
			// 保持水平位置由拖拽逻辑控制
			$divider.css({
				'position': 'absolute',
				'top': topPosition + 'px',
        ...
        // 监听滚动事件，动态调整位置
		$($summary).on('scroll', updateDividerPosition);
